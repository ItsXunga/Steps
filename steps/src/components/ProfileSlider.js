...
const ProfileSlider = (props) => {
  ...
  useEffect(() => {
    const resultsRotas = props.rotas.filter(function (value) {
      return value.creator && value.creator.name === props.user.name;
    });
    setSelectedTabArray(resultsRotas);

    // const resultsRotasFavoritas = props.favCircuits.filter(function (value) {
    //   return value.creator && value.creator.name === props.user.name;
    // });
    // setSelectedTabArray(resultsRotasFavoritas);
  }, [props.rotas, props.user]);
  ...
  const changeOwn = () => {
    setSelectedTabArray(
      props.rotas.filter(function (value) {
        return value.creator && value.creator.name === props.user.name;
      })
    );
